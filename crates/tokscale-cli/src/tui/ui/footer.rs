...
fn render_main_row(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    if !is_very_narrow {
        let count_label = match app.current_tab {
            ...
            Tab::Prices => format!(" ({} price models)", app.data.prices.len()),
            Tab::Thinking => format!(" ({} thinking models)", app.data.thinking.len()),
            Tab::Accounts => format!(" ({} accounts)", app.data.codex_accounts.len()),
            Tab::Quota => format!(
                " ({} ROI models)",
                app.data.quota_value.model_summaries.len()
            ),
